...
_PREFERENCE = re.compile(r"^(?:i\s+)?(?:prefer|like|love|hate|dislike)\s+(.+)$", re.IGNORECASE)
...
def extract_explicit_candidates(text: str) -> tuple[ExtractionCandidate, ...]:
    ...
    match = _EXPLICIT.match(normalized) or _EXPLICIT_SAVE.match(normalized)
    explicitly_requested = match is not None
    match = match or _PREFERENCE.match(normalized)
    if match is None:
        return ()
    ...
    candidate = _candidate_from_content(
        ...
        and _PREFERENCE.match(normalized) is not None,
    )
    try:
...
def extract_explicit_tool_candidate(text: str) -> ExtractionCandidate | None:
    ...
    if match is None:
        return None
    candidate = _candidate_from_content(match.group(1).strip().rstrip("."))
    try:
...
def _candidate_from_content(
    ...
    *,
    ...
    memory_type = (
        ...
        if preference or prefer_plain_statement
